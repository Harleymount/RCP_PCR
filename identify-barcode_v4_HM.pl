...
my $primers_fasta = '~/Desktop/output/scripts/vector_database/newconstseq.fa';
my $bar2num_file  = '~/Desktop/output/scripts/bar2num2.txt';
my $tag_assignment = '~/Desktop/output/scripts/tag_assignment.csv';
...
while(my($read,$value) = each %$data){
    ...
    # my $element;
    #$element->{p1} =$P1_seq;  # for seeing the output of the category calling to refine plate index finding
    #$element->{p2} =$P2_seq;
    #$element->{R} =$R_seq;
    #$data2 = $element;
    ...
    #$element->{p1} =$P1_seq;  # for seeing the output of the category calling to refine plate index finding
    #$element->{p2} =$P2_seq;
    #$element->{P1_num} =$P1_num;  # for seeing the output of the category calling to refine plate index finding
    ##$element->{P2_num} =$P2_num;
    #$element->{R_seq} =$R_seq;
    #$element->{C} =$C_seq;
    my $R_num  = ($R_seq  && length($R_seq)  < 12)? &barcode_row_matching($bar2num,$R_seq) :0;
    my $C_num  = ($C_seq  && length($C_seq)  < 12)? &barcode_row_matching($bar2num,$C_seq) :0;
    #$element->{Rnum} =$R_num;
    #$element->{Cnum} =$C_num;
    #$data2 = $element;
    #############################
    ...
    if($category && $P1_num*$P2_num*$R_num*$C_num){
        ...
        #print Dumper $tag_strct;
        #my $plate_name =$tag_strct->{$type1}->{$type2}->{$P_TAG};
        #print "$category\t$P_TAG\t$R_TAG\t$C_TAG\t$read\n";
        #$data2->{$category}->{$plate_name}->{$R_TAG}->{$C_TAG}->{$read} = $stat;
        if($tag_strct->{$type1}->{$type2}->{$P_TAG}){
            my $plate_name = $tag_strct->{$type1}->{$type2}->{$P_TAG};
            #print Dumper $plate_name;
            $data2->{$category}->{$plate_name}->{$R_TAG}->{$C_TAG}->{$read} = $stat;
            #print "$category\t$plate_name\t$R_TAG\t$C_TAG\t$read\n";
        }
    }
...
sub assign_category(){
    ...
    if($R1->{'PS1.0-primer'} && $R2->{'PS2.0-primer'}){
        if($R1->{'PS1.0-primer'}->{end} < 40 && $R2->{'PS2.0-primer'}->{end} < 40){
            $P_SEQ1 = substr $seq->{R1},$R1->{'PS1.0-primer'}->{str}-9,8;#test a change in this -10,9 to -8,8
            $P_SEQ2 = substr $seq->{R2},$R2->{'PS2.0-primer'}->{str}-18,8;#test a change in this -10,9 to -17,8
            $goto1 = 1;
        }
    }
...
sub barcode_matching(){
    ...
    my $seq     = shift;
    my @bar_count;
    #foreach my $key(keys %$bar2num){  print "DEBUG: [$key]=>[$$bar2num{$key}]\n"}
    ...
    for my $key (keys %$bar2num){
        ...
        $element->{max} //= 0; #### adding this prevents undef THIS LINE FIXED EVERYTHING
        push @bar_count, $element;
    
    }
    my $val;
    #@bar_count = sort{$b <=> $a} @bar_count;  #this wrks but wrong plate # the actual line doesnt see below
    #print Dumper \@bar_count;
    @bar_count = sort{$b ->{max} <=> $a->{max}} @bar_count;
    #my $called_bar = $bar_count[0]->{bar}; # i added
    # $val = $bar2num->{$called_bar}; # i added
...
sub barcode_row_matching(){
    my $row2num = shift;
    my $seq     = shift;
    my @bar_count;
    #foreach my $key(keys %$row2num){  print "DEBUG: [$key]=>[$$row2num{$key}]\n"}
    
    for my $key (keys %$row2num){
        my $count;
        for(my $k=0; $k<length($key)-1; $k++){
            my $k_nuc = substr $key,$k,2;
            for(my $i=0; $i<length($seq)-1; $i++){
    ...
    @bar_count = sort{$b ->{max} <=> $a->{max}} @bar_count;
    #my $called_bar = $bar_count[0]->{bar}; # i added
    # $val = $row2num->{$called_bar}; # i added
    if($bar_count[0]->{max} == $bar_count[1]->{max}){
        $val = 0;
    }elsif($bar_count[0]->{max}>5){ ########## THINK ABOUT THIS THRESHOLD
        my $called_bar = $bar_count[0]->{bar};
        $val = $row2num->{$called_bar};
    }else{
        $val = 0;
    }
    return $val
